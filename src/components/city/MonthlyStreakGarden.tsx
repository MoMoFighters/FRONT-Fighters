...
export default function MonthlyStreakGarden({
    ...
    const isInitialMonth =
        currentMonthStreak.year === year && currentMonthStreak.month === month;

    const streakQuery = useQuery({
        queryKey: userId
            ? ["city", "friend-streak", userId, year, month]
            : ["city", "my-streak", year, month],
        queryFn: async () => {
            ...
            if (!result.success || !result.data) {
                throw new Error(result.message ?? "잔디 정보를 불러오지 못했습니다.");
            }

            return result.data;
        },
        initialData: isInitialMonth ? currentMonthStreak : undefined,
        placeholderData: (previousData) => previousData,
    });

    const streakDays = useMemo(
        () => normalizeStreakData(streakQuery.data ?? {
            year,
            month,
            streaks: [],
        }),
        [month, streakQuery.data, year]
    );
    ...
    }, [streakQuery.error, streakQuery.isError]);
    ...
    return (
        <>
            <div className="absolute bottom-[11%] left-[5.5%] z-10 h-[13.3cqw] w-[15cqw] -rotate-4">
                <Image src={streakDate} alt="잔디 달력" fill className="object-contain" />
                ...
                <div className="absolute left-[52%] top-[42%] -translate-x-1/2 -translate-y-1/2">
                    ...
                    >
                        <button
                            type="button"
                            onClick={() => moveMonth(-1)}
                            disabled={streakQuery.isFetching}
                            aria-label="이전 달"
                        >
                            <ChevronLeft
                                className={`size-[1.33cqw] ${
                                    streakQuery.isFetching
                                        ? "text-slate-300"
                                        : "cursor-pointer text-slate-900 hover:scale-110"
                                }`}
                            />
                        </button>
                        ...
                        <button
                            type="button"
                            onClick={() => moveMonth(1)}
                            disabled={isCurrentMonth || streakQuery.isFetching}
                            aria-label="다음 달"
                        >
                            <ChevronRight
                                className={`size-[1.33cqw] ${
                                    ...
                                    isCurrentMonth || streakQuery.isFetching
                                        ? "text-slate-300"
                                        : "text-slate-900"
                                }`}
                            />
            ...
            <div
                className={`absolute bottom-[8%] left-[13%] z-10 grid grid-cols-7 grid-rows-5 gap-x-[0.04cqw] gap-y-[0.08cqw] rounded-sm bg-yellow-800/50 px-[0.04cqw] py-[0.6cqw] transition-opacity ${
                    streakQuery.isFetching ? "opacity-70" : "opacity-100"
                }`}
                style={{
