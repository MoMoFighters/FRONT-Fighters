...
export const readMessageService = async ({
    roomId,
    accessToken,
}: ReadMessageProps): Promise<BaseResponse<ReadMessageData>> => {
    try {
        const response = await fetch(
        ...
        const result: BaseResponse<ReadMessageData> =
            await response.json();
...
export const createChatRoomService = async ({
    userId,
    accessToken,
}: CreateChatRoomProps): Promise<BaseResponse<CreateChatRoomData>> => {
    try {
        const response = await fetch(
        ...
        const result: BaseResponse<CreateChatRoomData> =
            await response.json();
        ...
        if (response.status === 409)
            throw new Error(
                result.message ||
                '이미 채팅방이 존재하거나 자기 자신과는 채팅할 수 없습니다.'
            );
