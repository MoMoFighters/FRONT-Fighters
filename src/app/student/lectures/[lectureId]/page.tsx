...
export default async function LectureDetailPage({ params, searchParams }: LectureDetailPageProps) {
    ...
    return (
        <div className="p-12 relative">
            <MovePageBackBtn href="/student/lectures" />
            <LectureItem lecture={lecture} role="student" mode="detail" />
            <div className="mt-10 bg-white flex flex-col border rounded-lg border-slate-200 p-6 relative">
                <LectureDetailNav href={`/student/lectures/${lectureId}`} />
                <div className="border-t border-slate-400 mb-4" />
                <LectureDetailList role="student" chapters={lecture.chapters} reviews={paginatedReviews} />
            </div>
            {tab === "reviews" && (
