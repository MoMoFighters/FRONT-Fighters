...
export default function AdminUserReportCount({
    ...
    // TODO: 제재 누적 횟수 증감 API 명세가 확정되면 이 로컬 상태를 API 응답값으로 교체한다.
    ...
    return (
        <div className={`flex items-center ${compact ? "justify-start" : "justify-between"} gap-1.5`}>
            ...
            <div className="flex items-center gap-1">
                <Button type="button" variant="ghost" size="icon" aria-label="제재 누적 횟수 감소" onClick={() => setPendingDelta(-1)} className="size-6 rounded-md p-0 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                    <Minus className="size-3.5" />
                </Button>
                <Button type="button" variant="ghost" size="icon" aria-label="제재 누적 횟수 증가" onClick={() => setPendingDelta(1)} className="size-6 rounded-md p-0 text-indigo-500 hover:bg-indigo-50 hover:text-indigo-700">
