    ...
    return (
        <aside className="sticky top-20 self-start space-y-5">
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm [container-type:inline-size]">
                ...
                <p className="mt-2 text-[clamp(0.7rem,3cqw,0.75rem)] leading-5 text-slate-500">
                    강의는 누구나 둘러볼 수 있습니다. 수강 신청, 학습 기록 저장, 도시 성장은 로그인 후 이용할 수 있습니다.
                </p>

                <Link
                    href="/about"
                    className="mt-4 flex h-[clamp(2rem,9cqw,2.25rem)] w-full items-center justify-center rounded-lg bg-indigo-500/90 text-[clamp(0.7rem,3cqw,0.75rem)] font-bold text-white transition-colors hover:bg-indigo-500"
                >
                    사이트 소개 보러가기
                </Link>
            </section>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm [container-type:inline-size]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                    {categoryGuides.map((category) => (
                        <span
                            key={category}
                            className="rounded-full bg-slate-50 px-2.5 py-1 text-[clamp(0.625rem,2.5cqw,0.6875rem)] font-bold text-slate-500"
                        >
                            {category}
                        </span>
                    ))}
